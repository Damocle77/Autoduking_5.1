...
# ==============================================================================
# ducking_auto_cartoni.sh v1.7 - Audio Ottimizzato per Cartoni e Musical
#
...
# Parametri base per cartoni animati e musical - OTTIMIZZATO LG SP7 + SPK8
# NB. Front preservati per proiettori sonori Meridian
VOICE_BOOST=3.3
LFE_REDUCTION=0.79
LFE_DUCK_THRESHOLD=0.008
LFE_DUCK_RATIO=3.8
FX_DUCK_THRESHOLD=0.015
...
if [ $(awk "BEGIN {print ($LUFS < -20) ? 1 : 0}") -eq 1 ]; then
    ...
    echo "APPLICATO: Ducking front leggermente aumentato (+0.2) per chiarezza - preservando Meridian"
    ...
    echo "APPLICATO: Ducking front minimale (+0.1) per preservare proiezione spaziale"
    echo "APPLICATO: Makeup gain potenziato (+0.7) per livello finale ottimale"
else
    echo "APPLICATO: Parametri LG SP7 + SPK8 bilanciati - front preservati per Meridian"
...
echo "OTTIMIZZATO: Canali front preservati per tecnologia Meridian LG SP7 + SPK8"
